...
export const updateUserInformation = (name, email, phoneNumber, password) => async (dispatch) => {
    try {
        ...
        const {data} = await axios.put(
            `http://localhost:8000/api/user/update-user-info`,
            {
                email,
                password,
                phoneNumber,
...
export const updatUserAddress = (country, city, address1, address2,zipCode, addressType) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(
            `http://localhost:8000/api/user/update-user-addresses`,
            {
                country,
                city,
                address1,
...
export const deleteUserAddress = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `http://localhost:8000/api/user/delete-user-address/${id}`,
       { withCredentials: true,}
    );
...
export const getAllUsers = () => async (dispatch) => {
    try {
      ...
      const { data } = await axios.get(`http://localhost:8000/api/user/admin-all-users`, {
        withCredentials: true,
      });
